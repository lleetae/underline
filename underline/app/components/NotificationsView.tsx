...
export function NotificationsView({ onBack, onNavigateToMatch }: NotificationsViewProps) {
    ...
    const getNotificationIcon = (type: string) => {
        switch (type) {
            case 'match_request':
                return <Heart className="w-5 h-5 text-[#CC0000]" />;
            case 'match_accepted':
                return <Heart className="w-5 h-5 text-[#CC0000] fill-[#CC0000]" />;
            case 'contact_revealed':
                return <CreditCard className="w-5 h-5 text-[#CC0000]" />;
            default:
                return <Bell className="w-5 h-5 text-[#CC0000]" />;
        }
    };
        ...
        <div className="w-full max-w-md relative shadow-2xl shadow-black/5 min-h-screen bg-[#FAFAFA] flex flex-col">
            ...
            <div className="sticky top-0 z-10 bg-[#FAFAFA] border-b border-[#171717]/10">
                <div className="flex items-center justify-between px-6 py-4">
                    ...
                    <h1 className="font-serif text-2xl text-[#171717]">알림</h1>
                    {unreadCount > 0 && (
                        <button
                            onClick={markAllAsRead}
                            className="text-xs text-[#CC0000] font-sans hover:underline"
                        >
                            모두 읽음
                        </button>
                    )}
            ...
            <div className="flex-1 overflow-y-auto">
                {notifications.length === 0 ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-20 px-6">
                        <div className="w-16 h-16 rounded-full bg-[#CC0000]/10 flex items-center justify-center mb-4">
                            <Bell className="w-8 h-8 text-[#CC0000]/40" />
                        </div>
                        <h3 className="font-serif text-xl text-[#171717] mb-2">
                            알림이 없습니다
                        </h3>
                        <p className="text-sm text-[#171717]/60 font-sans text-center leading-relaxed">
                    ...
                    <div className="divide-y divide-[#171717]/5">
                        {notifications.map((notification) => (
                            <button
                                ...
                                className={`w-full px-6 py-4 flex gap-4 hover:bg-[#FAFAFA]/50 transition-colors text-left ${!notification.is_read ? 'bg-[#CC0000]/5' : ''
                                    }`}
                            >
                                {/* Sender Photo */}
                                <div className="flex-shrink-0">
                                    {notification.sender?.photo_urls_blurred?.[0] ? (
                                        <div className="w-12 h-12 rounded-full overflow-hidden border border-[#171717]/10">
                                            <ImageWithFallback
                                        ...
                                        <div className="w-12 h-12 rounded-full bg-[#171717]/5 border border-[#171717]/10 flex items-center justify-center">
                                            <User className="w-6 h-6 text-[#171717]/30" />
                                        </div>
                                    )}
                                </div>
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-start gap-2 mb-1">
                                        ...
                                        <p className="flex-1 text-sm text-[#171717] font-sans leading-relaxed">
                                            {getNotificationMessage(notification)}
                                        </p>
                                        {!notification.is_read && (
                                            <div className="flex-shrink-0 w-2 h-2 bg-[#CC0000] rounded-full mt-1" />
                                        )}
                                    </div>
