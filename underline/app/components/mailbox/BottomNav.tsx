...
export function BottomNav({ activeTab, onTabChange }: BottomNavProps) {
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-[#FCFCFA] border-t border-[#1A3C34]/10 px-6 py-3 z-50 flex justify-center">
      <div className="flex justify-between items-center w-full max-w-md">
        <button 
           onClick={() => onTabChange?.("home")}
           className={cn(
             "flex flex-col items-center gap-1 transition-colors p-2",
             activeTab === "home" ? "text-[#D4AF37]" : "text-[#1A3C34]/30"
           )}
        >
        ...
        <button 
           ...
           className={cn(
             "flex flex-col items-center gap-1 transition-colors p-2",
             activeTab === "mailbox" ? "text-[#D4AF37]" : "text-[#1A3C34]/30"
           )}
        >
        ...
        <button 
           ...
           className={cn(
             "flex flex-col items-center gap-1 transition-colors p-2",
             activeTab === "profile" ? "text-[#D4AF37]" : "text-[#1A3C34]/30"
           )}
        >
